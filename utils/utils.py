...
def load_dataset(dataset_path, train_own_data, train_ratio):
    ...
    if train_own_data:
        ...
        #   自己的数据集，遍历大循环
        #-------------------------------------------------------------#
        #os.listdir 是传入一个相应的路径后能够返回该目录下所有的文件名
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio = 0.05, warmup_lr_ratio = 0.1, no_aug_iter_ratio = 0.05, step_num = 10):
    ...
    if lr_decay_type == "cos":
        ...
        func =  ReduceLROnPlateau(monitor='val_accuracy',
                         factor=0.1,
                         patience=1,
                         verbose=1,
                         mode='auto',
...
def download_weights(backbone, model_dir="./model_data"):
    ...
    # src_file_list = glob(src_dir +"E:/研究生的课题项目/Siamese/model_data")                    # glob获得路径下所有文件，可根据需要修改
